...
for i=1:size(SNR,2)%单灯
        ...
        Var = Power/SNR(i) ;   %在该测试点下，PD旋转过程中收到的噪声干扰
            ...
            ['Work remain ' num2str(size(SNR,2)-i) '  (' num2str(1) '/' num2str(5) ')'] 
end
...
for i=1:size(SNR,2)%单灯
        ...
        Var = Power/SNR(i) ;   %在该测试点下，PD旋转过程中收到的噪声干扰
            ...
            ['Work remain' num2str(size(SNR,2)-i) '  (' num2str(2) '/' num2str(5) ')'] 
end
%%RPM K=6
N_pd = Facing_Vector_K( P_pd, P_led(1,:),6);  %PD旋转过程中的法线向量
for i=1:size(SNR,2)%单灯
        Power = Theory_Power(P_pd, P_led(1,:), N_pd, N_led, m, M, Ar);    %在该测试点下，PD旋转过程中接收到的功率(无抖动理想状态下)
        Var = Power/SNR(i) ;   %在该测试点下，PD旋转过程中收到的噪声干扰
            parfor k=1:test_time
            ...
            ['Work remain' num2str(size(SNR,2)-i) '  (' num2str(3) '/' num2str(5) ')'] 
end
...
for i=1:size(SNR,2)
        ...
        Var = Power/SNR(i) ;   %在该测试点下，PD旋转过程中收到的噪声干扰
            ...
            ['Work remain' num2str(size(SNR,2)-i) '  (' num2str(4) '/' num2str(5) ')'] 
end
...
Angle_Rot = 2*pi/4;   %旋转角度
N_pd = Facing_Vector( P_pd, P_led(1,:), Angle_Dev, Angle_Rot);  %PD旋转过程中的法线向量
midVector = [0 0 1] - N_pd(2,:);
midVector(3) = -midVector(3);
N_pd(4,:) = [0 0 1] +  midVector(3);
N_pd(5,:) = [0 0 1];
for i=1:size(SNR,2)%单灯
        Power = Theory_Power(P_pd, P_led(1,:), N_pd, N_led, m, M, Ar);    %在该测试点下，PD旋转过程中接收到的功率(无抖动理想状态下)
        Var = Power/SNR(i) ;   %在该测试点下，PD旋转过程中收到的噪声干扰
            parfor k=1:test_time
                Power_noise = Power +sqrt(Var).*randn(1,5);     %给接收功率加上噪声
                Estimated_Pos = PSO_Method(Power_noise, P_led(1,:), N_pd, N_led, m, M, Ar, num, num_cycle); %在该测试点下，算法所得到的估计位置
                MidErr(k) = norm((P_pd - Estimated_Pos), 2);  %累加，最后求平均 
            end
            Err(5,i) = mean(MidErr); %在该测试点下，平均定位误差
            ['Work remain ' num2str(size(SNR,2)-i) '  (' num2str(5) '/' num2str(5) ')'] 
end

figure(1)
semilogy(SNR_dB,Err(1,:).^2,'Color','#0072BD','LineStyle','-','Marker','*')
hold on
semilogy(SNR_dB,Err(5,:).^2,'Color','#0072BD','LineStyle','-','Marker','+')
hold on
semilogy(SNR_dB,Err(3,:).^2,'Color','#0072BD','LineStyle','-','Marker','>')
hold on
semilogy(SNR_dB,Err(4,:).^2,'Color','#0072BD','LineStyle','-','Marker','o')
hold on
semilogy(SNR_dB,Err(2,:).^2,'Color','#0072BD','LineStyle','-','Marker','<')
hold off
grid on
legend('HPTPS','HPFPS','RPM','RPM','RSS')
