...
P_led = [1.5 1.5 3; 0.5 0.5 3];  %LED的位置
...
for i=1:size(SNR,2)%文献Indoor Positioning System Based on Single LED Using Symmetrical Optical Receiver
        ...
        Var = Power/SNR(i) ;   %在该测试点下，PD旋转过程中收到的噪声干扰
            ...
            ['Work remain' num2str(size(SNR,2)-i) '  (' num2str(1) '/' num2str(4) ')'] 
end

Angle_Dev =pi/6;   %偏移角度
Angle_Rot = 2*pi/3;   %旋转角度
N_pd = Facing_Vector( P_pd, P_led(1,:), Angle_Dev, Angle_Rot);  %PD旋转过程中的法线向量
for i=1:size(SNR,2)%双灯
        Power = [Theory_Power(P_pd, P_led(1,:), N_pd, N_led, m, M, Ar) Theory_Power(P_pd, P_led(2,:), N_pd, N_led, m, M, Ar)];    %在该测试点下，PD旋转过程中接收到的双灯功率
        Var = Power/SNR(i) ;   %在该测试点下，PD旋转过程中收到的噪声干扰
            parfor k=1:test_time
                Power_noise = Power +sqrt(Var).*randn(1,6);     %给接收功率加上噪声
                Estimated_Pos = PSO_Method2lamp(Power_noise, [P_led(1,:);P_led(2,:)], N_pd, N_led, m, M, Ar, num, num_cycle); %在该测试点下，算法所得到的估计位置
                MidErr(k) = norm((P_pd - Estimated_Pos), 2);         %累加，最后求平均 
            end
            Err(2,i) = mean(MidErr); %在该测试点下，平均定位误差
            ['Work remain' num2str(size(SNR,2)-i) '  (' num2str(2) '/' num2str(4) ')'] 
end
...
for i=1:size(SNR,2)%三灯短距离
        ...
        Var = Power/SNR(i) ;   %在该测试点下，PD旋转过程中收到的噪声干扰
            ...
            ['Work remain' num2str(size(SNR,2)-i) '  (' num2str(3) '/' num2str(4) ')'] 
end
...
for i=1:size(SNR,2)
        ...
        Var = Power/SNR(i) ;   %在该测试点下，PD旋转过程中收到的噪声干扰
            ...
            ['Work remain' num2str(size(SNR,2)-i) '  (' num2str(4) '/' num2str(4) ')'] 
end
...
hold on
semilogy(SNR_dB,Err(2,:),'Color','#0072BD','LineStyle','-','Marker','o')
hold on
semilogy(SNR_dB,Err(3,:),'Color','#0072BD','LineStyle','-','Marker','+')
hold on
semilogy(SNR_dB,Err(4,:),'Color','r','LineStyle','-','Marker','+')
hold off
grid on
% title('The relation ship between Shanke distance and position accuracy')
legend('Single Lamp (RPM)','Dual Lamp (RPM)','Three Lamp (RPM)','Three Lamp (Tri)')%,'SNR 60dB','SNR 60dB(without shake)','SNR 80dB''SNR 80dB(without shake)')
xlabel('SNR (dB)')
ylabel('MSE')
toc