...
for i = 1: 1: num
    %每个粒子
    particle(i,1) = i;%编号
%     particle(i,4) = (0.5*randn(1,1)+1.5);%z=0~3
%     if particle(i,4)>=2.5
%         particle(i,4)=2.5;
%     elseif particle(i,4)<=0
%         particle(i,4)=0;
%     end
%     particle(i,2) = (rand(1,1)-0.5)*2*sqrt(3)*(3-particle(i,4))+1.5;%x=0~3
%     particle(i,3) = sqrt(3*(3-particle(i,4))^2-(1.5-particle(i,2))^2);
%     particle(i,3) = (rand(1,1)-0.5)*2*particle(i,3)+1.5;%y=0~3
...
for cycle=1:1:num_cycle%粒子群运动次数
    ...
    for i = 1: 1: num%每个粒子
        ...
        v_id = w*v_id + c1*r1*(p_id-x_id)*exp(-r1*m) + c2*r2*(pg-x_id);
        ...
        particle(i,8) = norm((Theory_Power(particle(i,2:4), P_led, N_pd, N_led, m, M, Ar)-Receive_power),2);%记录适应值
    end
    pg=tmp_pg;  
 %   particle(particle(:,8)==max(particle(:,8)),2:4)=mean(particle(particle(:,8)==min(particle(:,8)),2:4),1);
end
% multi_pg = particle(particle(:,8)==0,2:4);
% pg = transpose(mean(transpose(multi_pg)));
advalue = particle(:,8).^(-5);
advalue = advalue./sum(advalue);
pg(1) = sum(advalue.*particle(:,2));
pg(2) = sum(advalue.*particle(:,3));
pg(3) = sum(advalue.*particle(:,4));
Calculated_point = pg;
end
