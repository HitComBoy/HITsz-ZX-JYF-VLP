...
%     particle(i,7) = -v_limit+v_limit*2*rand(1,1);%vz=-v_limit~v_limit
    ...
    l_power = norm((Theory_Pow-Receive_power),2);%当前粒子位置下接收功率与实际接收功率之差
    g_power = norm((GlocalBest_Theory_Pow-Receive_power),2);
    if  l_power < g_power
        pg = particle(i,2:4);
    end
end
...
%         end
        ...
        GlocalBest_Theory_Pow = GlocalBest_Theory_Pow(2:size(GlocalBest_Theory_Pow),:); 
        l_power = norm((Theory_Pow-Receive_power),2);%当前粒子位置下接收功率与实际接收功率之差
        g_power = norm((LocalBest_Theory_Pow-Receive_power),2);
        if  l_power < g_power
            pi(i,2:4) = particle(i,2:4);
        end
        %刷新群体历史最优
        g_power = norm((GlocalBest_Theory_Pow-Receive_power),2);%所有粒子移动过程中接收功率与实际接收功率之差的最小值
        if l_power < g_power
