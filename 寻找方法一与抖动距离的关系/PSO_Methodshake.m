...
hold on
semilogy(Shakedistance,repmat((Err(1,1)*0.98).^2,1,size(Shakedistance,2)),'Color','#0072BD','LineStyle','--')
hold on
semilogy(Shakedistance,Err(2,:).^2,'Color','#D95319','LineStyle','-')
hold on
semilogy(Shakedistance,repmat((Err(2,1)*0.98).^2,1,size(Shakedistance,2)),'Color','#D95319','LineStyle','--')

hold off
grid on
title('The relation ship between Shanke distance and position accuracy')
legend('SNR 40dB','SNR 40dB(without shake)','SNR 60dB','SNR 60dB(without shake)')
