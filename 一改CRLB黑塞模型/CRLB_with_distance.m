...
Ar = 1;
A = (m+1)*Ar/(2*pi);
test_point = [1.5 1.5 3];
N_led = [0 0 -1];
P_led = [1.5 1.5 3];
SNR = 30:10:60;
pi = 3.14159;
CRLB = zeros(length(SNR),round(2/step_len+1));
for i=1:length(SNR)


        for j=0:step_len:2
            Var_Noise = Theory_Power([test_point(1) test_point(1) j], P_led, P_led-[test_point(1) test_point(1) j], N_led, m, M, Ar)/10^(SNR(i)/10);
            delta = sqrt(Var_Noise);
            x =  test_point(1) - P_led(1);
            y =  test_point(2) - P_led(2);
            z =  j - 3;
            N_pd = Facing_Vector([test_point(1) test_point(2) j], P_led);

            n(:,:,1) = -transpose(N_led)*N_pd(1,:);
            n(:,:,2) = -transpose(N_led)*N_pd(2,:);
            n(:,:,3) = -transpose(N_led)*N_pd(3,:);

            H1 = Theory_Power([test_point(1) test_point(1) j], P_led, N_pd(1,:), N_led, m, M, Ar);
            H2 = Theory_Power([test_point(1) test_point(1) j], P_led, N_pd(2,:), N_led, m, M, Ar);
            H3 = Theory_Power([test_point(1) test_point(1) j], P_led, N_pd(3,:), N_led, m, M, Ar);
            ...
            I = -FisherImatrix;
            B = inv(I);
            CRLB(i,round(j/step_len+1)) = trace(B);
        end
end

Distance = repmat(3,1,round(2/step_len+1))-[0:step_len:2] ;
figure
semilogy(Distance,transpose(CRLB(1,:))./transpose(Distance),'b');
hold on 
semilogy(Distance,transpose(CRLB(2,:))./transpose(Distance),'g');
hold on 
semilogy(Distance,transpose(CRLB(3,:))./transpose(Distance),'r');
hold on 
semilogy(Distance,transpose(CRLB(4,:))./transpose(Distance),'k');

hold on 
semilogy(Distance,transpose(CRLB(1,:)),'b--');
hold on 
semilogy(Distance,transpose(CRLB(2,:)),'g--');
hold on 
semilogy(Distance,transpose(CRLB(3,:)),'r--');
hold on 
semilogy(Distance,transpose(CRLB(4,:)),'k--');


legend('SNR = 30dB(Normalize)','SNR = 40dB(Normalize)','SNR = 50dB(Normalize)','SNR = 60dB(Normalize)','SNR = 30dB','SNR = 40dB','SNR = 50dB','SNR = 60dB')
xlabel('Distance to LED')
ylabel('(Normalize) MSE')
title('The relationship between CRLB and distance')
grid on
