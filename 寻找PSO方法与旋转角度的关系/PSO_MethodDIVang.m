...
Angle_Dev = pi/18:pi/36:pi/3;   %偏移角度
...
test_time = 10000;                     %在特定粒子群数量下，定位误差测试次数（最后取平均）
...
for i=1:size(Angle_Dev,2)
        for j=1:size(SNR,2)
            N_pd = Facing_Vector( P_pd, P_led, Angle_Dev(i), Angle_Rot);  %PD旋转过程中的法线向量
        ...
        ['Work remain' num2str(size(Angle_Dev,2)*size(SNR,2)-((i-1)*size(SNR,2)+j))] 
end

figure(1)
semilogy(Angle_Dev,Err(:,1).^2)
hold on
semilogy(Angle_Dev,(Err(:,1)*0.97).^2)
hold on
semilogy(Angle_Dev,Err(:,2).^2)
hold on
semilogy(Angle_Dev,(Err(:,2)*0.97).^2)
hold off
grid on
legend('SNR40','SNR60')
xlim([0.1745 1.0472])
