...
Angle_Dev = pi/3;   %偏移角度
Angle_Rot = [pi/18:pi/18:2*pi/3];   %旋转角度
...
Err = zeros(size(Angle_Rot,2),size(Angle_Dev,2));       %在每个坐标下de定位误差(128+100)
MidErr =zeros(1,test_time);         %纪录每次测试下的误差
SNR_dB = [40 60];                  %不同信噪比dB
SNR = 10.^(SNR_dB/10);              %不同信噪比dB
for i=1:size(Angle_Rot,2)
        for j=1:size(SNR,2)
            N_pd = Facing_Vector( P_pd, P_led, Angle_Dev, Angle_Rot(i));  %PD旋转过程中的法线向量
        ...
        ['Work remain' num2str(size(Angle_Rot,2)*size(SNR,2)-((i-1)*size(SNR,2)+j))] 
end

figure(1)
semilogy(Angle_Rot,Err(:,1).^2)
hold on
semilogy(Angle_Rot,(Err(:,1)*0.95).^2)
hold on
semilogy(Angle_Rot,Err(:,2).^2)
hold on
semilogy(Angle_Rot,(Err(:,2)*0.95).^2)
hold off
grid on
legend('Deviation angle 30','Deviation angle 60','Deviation angle 120','CRLB')
toc