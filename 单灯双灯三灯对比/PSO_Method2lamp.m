function Calculated_point = PSO_Method2lamp(Receive_power, P_led, N_pd, N_led, m, M, Ar, num, num_cycle)
%*********************************************************************************************%
...
for i = 1: 1: num
    ...
    %群体历史最优初值
    l_power = norm(([Theory_Power(particle(i,2:4), P_led(1,:), N_pd, N_led, m, M, Ar) Theory_Power(particle(i,2:4), P_led(2,:), N_pd, N_led, m, M, Ar)]-Receive_power),2);%当前粒子位置下接收功率与实际接收功率之差
    g_power = norm(([Theory_Power(pg, P_led(1,:), N_pd, N_led, m, M, Ar) Theory_Power(pg, P_led(2,:), N_pd, N_led, m, M, Ar)]-Receive_power),2);%当前粒子位置下接收功率与实际接收功率之差
    if  l_power < g_power
        pg = particle(i,2:4);
    end
end
...
for cycle=1:1:num_cycle%粒子群运动次数
    ...
    for i = 1: 1: num%每个粒子
        ...
        %刷新个体历史最优
        l_power = norm(([Theory_Power(particle(i,2:4), P_led(1,:), N_pd, N_led, m, M, Ar) Theory_Power(particle(i,2:4), P_led(2,:), N_pd, N_led, m, M, Ar)]-Receive_power),2);%当前粒子位置下接收功率与实际接收功率之差
        g_power = norm(([Theory_Power(pi(i,2:4), P_led(1,:), N_pd, N_led, m, M, Ar) Theory_Power(pi(i,2:4), P_led(2,:), N_pd, N_led, m, M, Ar)]-Receive_power),2);%当前粒子移动过程中接收功率与实际接收功率之差的最小值
        if  l_power < g_power
            pi(i,2:4) = particle(i,2:4);
        end
        %刷新群体历史最优
        l_power = norm(([Theory_Power(particle(i,2:4), P_led(1,:), N_pd, N_led, m, M, Ar) Theory_Power(particle(i,2:4), P_led(2,:), N_pd, N_led, m, M, Ar)]-Receive_power),2);%当前粒子位置下接收功率与实际接收功率之差
        g_power = norm(([Theory_Power(tmp_pg, P_led(1,:), N_pd, N_led, m, M, Ar) Theory_Power(tmp_pg, P_led(2,:), N_pd, N_led, m, M, Ar)]-Receive_power),2);%所有粒子移动过程中接收功率与实际接收功率之差的最小值
        if l_power < g_power
